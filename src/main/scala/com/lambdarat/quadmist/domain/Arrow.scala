...
object Arrow extends StringEnum[Arrow] with StringCirceEnum[Arrow] {
  ...
  /**
    * Check that arrow list is a valid one
    *
  ...
  /**
    * Get the target coordinates of the arrow, given the center.
    *
  ...
  def compress(arrows: List[Arrow]): Option[Byte] = {
    ...
    val correctNumberOfArrows = arrows.size <= MAX_ARROWS

    Option.when(allArrowsAreDistinct && correctNumberOfArrows) {
